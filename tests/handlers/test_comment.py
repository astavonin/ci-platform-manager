...
class TestParseDiffLines:
    ...
    def test_empty_list_returns_empty_set(self) -> None:
        """Empty diffs list produces an empty set."""
    ...
    def test_single_diff_object_yields_correct_pairs(self) -> None:
        ...
        result = _parse_diff_lines(diffs)
        assert ("src/foo.py", 1) in result

    def test_multiple_diff_objects_produce_union(self) -> None:
        """Multiple diff objects contribute their pairs to the union."""
        diffs = [
    ...
    def test_missing_new_path_is_skipped(self) -> None:
        ...
        diffs = [{"diff": "@@ -0,0 +1 @@\n+x\n"}]
        assert _parse_diff_lines(diffs) == set()

    def test_empty_diff_text_is_skipped(self) -> None:
        """Diff object with empty diff string is silently skipped."""
        diffs = [{"new_path": "c.py", "diff": ""}]
        assert _parse_diff_lines(diffs) == set()
...
class TestFetchMrDiffLines:
    ...
    def test_happy_path_returns_correct_set(self, mock_run: MagicMock) -> None:
        """Valid JSON response from glab is parsed and returns the correct set."""
...
class TestPostInlineFindingsDiffFetch:
    ...
    def test_fetch_diff_lines_called_once_per_invocation(
        ...
        mock_fetch_diff.return_value = {("a.py", 1)}
